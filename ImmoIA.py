...
scale_lr.fit(X_train_pca,y_train)
scaled_pred = scale_lr.predict(X_test_pca)
score_scaled = scale_lr.score(X_test_pca,y_test)
print("StandardScaler + LR : ",score_scaled)

minmax_lr.fit(X_train_pca,y_train)
minmax_lr_pred = minmax_lr.predict(X_test_pca)
score_minmax = minmax_lr.score(X_test_pca,y_test)
print("MinMax + LR : ",score_minmax)
...
for i in range(4,6):
    ...
    #DecisionTreeRegressor
    decision_tr = DecisionTreeRegressor(max_depth=i)
    decision_tr.fit(X_train_pca,y_train)
    pred_dtr = decision_tr.predict(X_test_pca)
    score_dtr = decision_tr.score(X_test_pca,y_test)
    print(f"AD {i} : ",score_dtr)


    #StandardScaler + DecisionTreeRegressor
    std_dtr = make_pipeline(StandardScaler(),DecisionTreeRegressor(max_depth=i))
    std_dtr.fit(X_train_pca,y_train)
    std_dtr_pred = std_dtr.predict(X_test_pca)
    score_std_dtr = std_dtr.score(X_test_pca,y_test)
    print(f"StandardScaler + AD {i} : ",score_std_dtr)

    #MinMaxScaler + DecisionTreeRegressor
    minmax_dtr = make_pipeline(MinMaxScaler(),DecisionTreeRegressor(max_depth=i))
    minmax_dtr.fit(X_train_pca,y_train)
    minmax_dtr_pred = minmax_dtr.predict(X_test_pca)
    score_minmax_dtr = minmax_dtr.score(X_test_pca,y_test)
    print(f"MinMaxScaler + AD {i} : ",score_minmax_dtr)
print("\n")
...
for i in range(4,6):
    ...
    k_neigbors = KNeighborsRegressor(n_neighbors=i)
    k_neigbors.fit(X_train_pca,y_train)
    k_neigbors_pred = k_neigbors.predict(X_test_pca)
    score_kn = k_neigbors.score(X_test_pca,y_test)
    print(f"KNeighborsR {i} : " , score_kn)

    k_neigbors_std = make_pipeline(StandardScaler(),KNeighborsRegressor(n_neighbors=i))
    k_neigbors_std.fit(X_train_pca,y_train)
    k_neigbors_std_pred = k_neigbors_std.predict(X_test_pca)
    k_neigbors_std_score = k_neigbors_std.score(X_test_pca,y_test)
    print(f"StandardScaler + KNeighborsR {i} : ",k_neigbors_std_score)

    k_neigbors_minmax = make_pipeline(MinMaxScaler(),KNeighborsRegressor(n_neighbors=i))
    k_neigbors_minmax.fit(X_train_pca,y_train)
    k_neigbors_minmax_pred = k_neigbors_minmax.predict(X_test_pca)
    k_neigbors_minmax_score = k_neigbors_minmax.score(X_test_pca,y_test)
    print(f"MinMaxScaler + KNeighborsR {i} : ",k_neigbors_minmax_score)
